...
class BRB_CR_F:    
    d21_ref_val = [1.0, 0.5, 0.0]
    d22_ref_val = [2.0, 1.0, 0.0]
    ...
    def __init__(self, modelType, isDebug=False):
    ...
    def runBRB(self, inputs):
        transformed_input_d21 = self.transformInput(inputs[0], self.d21_ref_val)
        transformed_input_d22 = self.transformInput(inputs[1], self.d22_ref_val)
                
        self.consequentBeliefDegree = self.initRuleBase()
        
        self.calculateActivationWeight(transformed_input_d21, transformed_input_d22)
        self.updateBeliefDegree(transformed_input_d21, transformed_input_d22)
    ...
    def calculateActivationWeight(self, transformed_input_d21, transformed_input_d22):
        self.matchingDegree.clear()
        self.activationWeight.clear()
        
        if self.modelType == ModelType.DISJUNCTIVE_BRB:
            for i in range(len(transformed_input_d21)):
                self.matchingDegree.append(pow(transformed_input_d21[i], 1) + pow(transformed_input_d22[i], 1))

        elif self.modelType == ModelType.CONJUNCTIVE_BRB:
            for i in range(len(transformed_input_d21)):
                for j in range(len(transformed_input_d22)):
                    self.matchingDegree.append(pow(transformed_input_d21[i], 1) * pow(transformed_input_d22[j], 1))
    ...
    def updateBeliefDegree(self, transformed_input_d21, transformed_input_d22):
        beliefDegreeChangeLevel = (np.sum(transformed_input_d21) + np.sum(transformed_input_d22))/self.numberOfAntAttributes
